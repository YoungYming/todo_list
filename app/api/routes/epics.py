...
from app.services.completion_service import complete_all_epic_tasks, reopen_epic_tasks
...
@router.post("/{epic_id}/complete_all")
def post_complete_all(epic_id: int, db: Session = Depends(get_db)):
    """将该 Epic 下全部未完成子任务一次性标记完成（拖到「已完成」并人工确认后调用）。"""
    epic = db.query(Epic).filter(Epic.id == epic_id).first()
    if not epic:
        raise HTTPException(status_code=404, detail="Epic not found")
    count = complete_all_epic_tasks(epic_id, db)
    db.refresh(epic)
    return {"epic_id": epic_id, "completed": count, "progress": epic.progress}


@router.post("/{epic_id}/reopen")
def post_reopen(epic_id: int, db: Session = Depends(get_db)):
    """把该 Epic 从「已完成」移回：重新打开其全部已完成子任务。"""
    epic = db.query(Epic).filter(Epic.id == epic_id).first()
    if not epic:
        raise HTTPException(status_code=404, detail="Epic not found")
    count = reopen_epic_tasks(epic_id, db)
    db.refresh(epic)
    return {"epic_id": epic_id, "reopened": count, "progress": epic.progress}
