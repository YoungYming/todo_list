(function () {
  ...
  var __sanitize = null;  // 暴露 sanitizeModalState 供 bfcache 恢复时调用（避免重复绑定监听）
  function log() {
  ...
  // 局部刷新今日待办列表（拆分/编辑 Epic、清空列表后调用），不重载整页
  window.reloadTodayPanel = function () {
    var host = document.getElementById('today-refreshable');
    if (!host) return;
    fetch('/app/today/partial', { credentials: 'same-origin' })
      .then(function (res) { return res.ok ? res.text() : Promise.reject(new Error('刷新失败')); })
      .then(function (html) { host.innerHTML = html; })
      .catch(function () { /* 静默失败，避免打断主流程 */ });
  };
  ...
  function init() {
    ...
    // 暴露给 pageshow（bfcache 恢复）使用，仅重置弹窗状态而不重复绑定监听
    __sanitize = sanitizeModalState;
    // 初始化时强制关闭弹窗，避免浏览器恢复旧 UI 状态导致 taskId 丢失
    sanitizeModalState();

    // 完成按钮：用事件委托，列表局部刷新后仍然有效
    document.addEventListener('click', function (e) {
      var btn = e.target.closest('.btn-complete');
      if (btn) openModal(btn.getAttribute('data-task-id'), btn.getAttribute('data-title'));
    });
    ...
    form.addEventListener('submit', function (e) {
      ...
      fetch(url, {
        ...
        .then(function () {
          ...
          if (row) row.remove();
          if (document.querySelectorAll('.task-item').length === 0 && window.reloadTodayPanel) {
            window.reloadTodayPanel();
          }
          toast('提交成功', 'success');
          // 通知其它面板（Epic 看板/白板）：任务已完成
          document.dispatchEvent(new CustomEvent('todo:changed', { detail: { source: 'task-complete', taskId: taskId } }));
        })
        .catch(function (err) {
  ...
  try {
    ...
    // 浏览器前进/后退恢复（bfcache）时，仅重置弹窗状态，避免重复绑定监听导致重复提交
    window.addEventListener('pageshow', function () {
      if (__sanitize) __sanitize();
      else init();
    });
  } catch (e) {
